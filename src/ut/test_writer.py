...
def extract_imports_and_functions(test_code: str, assume_single_function=False) -> tuple[set, list]:
    """
    ...
    for line in lines:
        ...
        # Handle decorators
        elif line.strip().startswith("@") and not (in_function and assume_single_function):
            if current_function:
        ...
        elif line.startswith("def ") and not (in_function and assume_single_function):
            if current_function:
                # Save the previous function
                functions.append("\n".join(current_function))
                current_function = []
...
# This function is not used in collab mode
